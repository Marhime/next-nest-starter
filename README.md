...
- générer un exemple `.env` complet pour `apps/api` et `apps/web` (avec placeholders),
- ajouter un script d'aide pour initialiser Postgres local via Docker Compose,
- ou simplifier les étapes de migration/seed dans un script `setup.sh`.

Fin — README mis à jour.
